...
from esm.model.esm2_aa import ESM2_AA


def _has_regression_weights(model_name):
    """Return whether we expect / require regression weights;
    Right now that is all models except ESM-1v, ESM-IF, and partially trained ESM2 models"""
    # return not ("esm1v" in model_name or "esm_if" in model_name or "270K" in model_name or "500K" in model_name or "aa" in model_name or "AA" in model_name)
...
def load_model_and_alphabet_local(model_location, model_name=None):
...
def _load_model_and_alphabet_core_esm_aa(model_data):
    def upgrade_state_dict(state_dict):
        """Removes prefixes 'model.encoder.sentence_encoder.' and 'model.encoder.'."""
        prefixes = ["encoder.sentence_encoder.", "encoder."]
        pattern = re.compile("^" + "|".join(prefixes))
        ...
        state_dict = {
            sname: svalue
            for sname, svalue in state_dict.items()
            if "version" not in sname
        }
        return state_dict
    ...
    state_dict = model_data["model"]
    state_dict = upgrade_state_dict(state_dict)
    alphabet = esm.data.Alphabet.from_architecture("ESM-AA")
    model = ESM2_AA(
        num_layers=cfg.encoder_layers,
        embed_dim=cfg.encoder_embed_dim,
        attention_heads=cfg.encoder_attention_heads,
...
def load_model_and_alphabet_core(model_name, model_data, regression_data=None):
    if regression_data is not None:
        if "model" in regression_data:
            model_data["model"].update(regression_data["model"])
        else:
            model_data["model"].update(regression_data)

    if "esm-aa" in model_name.lower():
        model, alphabet, model_state = _load_model_and_alphabet_core_esm_aa(model_data)
        # return model, alphabet, model_state
...
def esm_aa_t12_35M(model_path):
    """12 layer ESM-AA model with 35M params.

    Returns a tuple of (Model, Alphabet).
    """
    return load_model_and_alphabet_local(model_path, model_name='ESM-AA')
